...
function onUpdatePost()
	...
	dadIconSize = dadIconSize - iconSpeed;

	if iconSize < 1 then
		iconSize = 1;
	end

	if dadIconSize < 1 then
		dadIconSize = 1;
	end

	scaleObject('dadIcon', dadIconSize, dadIconSize);
...
beat=beat+1
	...
	if getProperty('health') < 0.4150 then
		triggerEvent('Screen Shake','0.5, 0.0035','1, 0.0025')
	end
end
